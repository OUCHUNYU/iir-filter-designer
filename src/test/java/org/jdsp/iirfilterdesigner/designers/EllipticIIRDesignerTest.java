...
public class EllipticIIRDesignerTest {
    ...
    /**
     * an instance of {@link EllipticIIRDesigner} needed to call all the tested
     * methods.
     */
    ...
    /**
     * Test method for
     * {@link EllipticIIRDesigner#designDigitalFilter(org.signalml.math.iirdesigner.FilterType, double[], double[], double, double) }.
     */
    @Test
    public void testDesignDigitalFilter() throws BadFilterParametersException {
        ...
        coeffs = iirdesigner.designFilter(FilterType.LOWPASS, new double[]{0.1}, new double[]{0.2}, 3.0, 40.0,
                false);
        ...
        coeffs = iirdesigner.designDigitalFilter(FilterType.HIGHPASS, new double[]{0.7}, new double[]{0.3}, 3.0,
                40.0);
        assertEquals(new FilterCoefficients(pythonB, pythonA), coeffs, 1e-4);

        // bandpass test
        pythonB = new double[]{0.02114251, 0.01792455, -0.00243741, 0.0, 0.00243741, -0.01792455, -0.02114251};
        pythonA = new double[]{1.0, 1.73921561, 3.38814582, 3.10556401, 3.01265939, 1.35579533, 0.69028645};

        coeffs = iirdesigner.designDigitalFilter(FilterType.BANDPASS, new double[]{0.5, 0.7},
                new double[]{0.3, 0.9}, 3.0, 40.0);
        assertEquals(new FilterCoefficients(pythonB, pythonA), coeffs, 1e-4);

        // bandstop test
        pythonB = new double[]{0.0987149, 0.16661581, 0.31652632, 0.30320545, 0.31652632, 0.16661581, 0.0987149};
        pythonA = new double[]{1.0, 0.68543316, -0.66268689, 0.05276535, 0.9103521, -0.10176143, -0.41718278};

        coeffs = iirdesigner.designDigitalFilter(FilterType.BANDSTOP, new double[]{0.3, 0.9},
                new double[]{0.5, 0.7}, 3.0, 40.0);
        assertEquals(new FilterCoefficients(pythonB, pythonA), coeffs, 1e-4);
    ...
    /**
     * Test method for
     * {@link EllipticIIRDesigner#calculatePrototype(int, double, double) }.
     */
    @Test
    public void testCalculatePrototype() throws BadFilterParametersException {
        ...
        FilterZerosPolesGain zpk2 = new FilterZerosPolesGain(new Complex[0],
                new Complex[]{new Complex(-1.3075602715790791, 0.0)}, 1.3075602715790791);
        assertEquals(zpk1, zpk2);

        // another test (even filter order)
        zpk1 = iirdesigner.calculatePrototype(4, 2, 3);

        Complex[] zeros = new Complex[]{new Complex(0, 1.06863478), new Complex(0, -1.06863478),
                new Complex(0, 1.00007796), new Complex(0, -1.00007796)};
        Complex[] poles = new Complex[]{new Complex(-6.62726745e-02, -1.00672153),
                new Complex(-6.62726745e-02, +1.00672153), new Complex(-7.22990126e-05, -1.00001461),
                new Complex(-7.22990126e-05, 1.00001461)};
        ...
        zeros = new Complex[]{new Complex(0, 1.58139008), new Complex(0, -1.58139008), new Complex(0, 1.16628458),
                new Complex(0, -1.16628458)};
        poles = new Complex[]{new Complex(-0.25129106, 0), new Complex(-0.13168110, -0.74196202),
                new Complex(-0.13168110, 0.74196202), new Complex(-0.02642856, -0.98568577),
                new Complex(-0.02642856, 0.98568577)};
    ...
    /**
     * Test method for
     * {@link EllipticIIRDesigner#calculateBandstopObjectiveFunctionValue(double, org.signalml.math.iirdesigner.AbstractIIRDesigner.BandstopObjectiveFunction) }.
     */
    @Test
    public void testBandstopObjectiveFunction() {
        ...
        // elliptic test 2
        bo = iirdesigner.new BandstopObjectiveFunction(0, new double[]{0.5, 0.8}, new double[]{0.55, 0.7}, 0.5,
                100);
        assertEquals(11.571836975873939, bo.value(0.00), 1e-4);

        // elliptic test 3
        bo = iirdesigner.new BandstopObjectiveFunction(0, new double[]{0.5, 0.8}, new double[]{0.55, 0.7}, 2.5,
                130);
    ...
    /**
     * Test method for
     * {@link EllipticIIRDesigner#calculateNaturalFrequency(org.signalml.math.iirdesigner.FilterType, int, double, double, double, double, boolean) }.
     */
    @Test
    public void testCalculateNaturalFrequency() throws BadFilterParametersException {
        ...
        // test 1
        wn = iirdesigner.calculateNaturalFrequency(FilterType.BANDPASS, 7, new double[]{0.6, 0.7},
                new double[]{0.55, 0.8}, 0.5, 100, true);
        assertEquals(new double[]{0.6, 0.7}, wn, 1e-16);

        // test 2
        wn = iirdesigner.calculateNaturalFrequency(FilterType.BANDSTOP, 3, new double[]{0.3, 0.9},
                new double[]{0.5, 0.7}, 3.0, 40.0, true);
        assertEquals(new double[]{0.38888824, 0.89999518}, wn, 1e-4);

        // test 3
        wn = iirdesigner.calculateNaturalFrequency(FilterType.BANDSTOP, 3, new double[]{0.3, 0.9},
                new double[]{0.5, 0.7}, 3.0, 40.0, false);
        assertEquals(new double[]{0.30000218, 0.83829193}, wn, 1e-5);

        // test 4
        wn = iirdesigner.calculateNaturalFrequency(FilterType.BANDSTOP, 8, new double[]{0.5, 0.8},
                new double[]{0.55, 0.7}, 0.5, 100, true);
    ...
    /**
     * Test method for
     * {@link EllipticIIRDesigner#calculateFilterOrder(org.signalml.math.iirdesigner.FilterType, double[], double[], double, double, boolean) }.
     */
    @Test
