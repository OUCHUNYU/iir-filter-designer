...
public class FourierTransform {
     ...
     * @param parameter  the parameter for the specified window
     */
    ...
    /**
     * Pads the given signal so that its length is a power of two (or performs
     * no padding if its length is already a power of two).
     *
    ...
    /**
     * Returns frequencies for elements returned by the
     * {@link FourierTransform#forwardFFT(double[])}.
     *
     * @param signal
    ...
    public static double[] getFrequencies(double[] signal, double samplingFrequency) {
        ...
        /*
         * /double step = samplingFrequency / N; double[] result = new
         * double[N]; for (int i = 0; i < (N + 1) / 2; i++) { result[i] = step *
         * i; } for (int i = (N + 1) / 2; i < N; i++) { result[i] = -(step * (N
         * - i)); } return result;
         */
    }

    /**
     * Calculates the power spectrum of the provided real sample data using FFT.
     *
    ...
    public double[] calculatePowerSpectrum(double[] samples) {
        ...
        for (int i = 1; i < size; ++i) {
            powerSpectrum[i] = square(fftTransformedData[i].getReal()) + square(fftTransformedData[i].getImaginary())
