...
/**
 * This class represents a designer which is capable of designing a Chebyshev
 * II.
 */
public class Chebyshev2IIRDesigner extends ChebyshevIIRDesigner {
     ...
     * Chebyshev II filter which meets the given specification.
     *
    ...
    /**
     * Returns zeros, poles, and gain of an normalized prototype analog lowpass
     * Chebyshev II filter which meets the specification.
     *
     * @param filterOrder the order of the prototype
     * @param gstop       the minimum attenuation in the stopband [dB]
     * @return zeros, poles and gain of the filter prototype which meets the
    ...
    protected double[] calculateNaturalFrequency(FilterType type, int filterOrder, double[] wp, double[] ws,
        ...
        double newFrequency = Math
                .cosh(1.0 / (filterOrder) * SpecialMath.acosh(Math.sqrt((GSTOP - 1.0) / (GPASS - 1.0))));
        ...
        if (type.isLowpass())
            ...
            wn[0] = (newFrequency / 2.0 * (passb[0] - passb[1])) + Math
                    .sqrt(newFrequency * newFrequency * Math.pow(passb[1] - passb[0], 2) / 4.0 + passb[1] * passb[0]);
