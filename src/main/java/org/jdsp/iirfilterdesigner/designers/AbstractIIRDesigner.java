...
import org.jdsp.iirfilterdesigner.exceptions.FilterOrderTooBigException;
...
public abstract class AbstractIIRDesigner {
    ...
    /**
     * Maximum filter order which can be designed using this designer. If a user
    ...
    /**
     * Undoes the warping that the
     * {@link AbstractIIRDesigner#prewarpFrequencies(double[]) does}.
     *
    ...
    /**
     * Normalizes the frequencies from 0 to 1 (1 corresponds to PI radians per
     * sample) for the given sampling frequency.
     *
     ...
     * @return coefficients of the filter which meets the given specification
     */
    public FilterCoefficients designDigitalFilter(double samplingFrequency, FilterType type, double[] passb,
                                                  double[] stopb, double gpass, double gstop) throws BadFilterParametersException {
     ...
     *
     * @param type   type of the filter (lowpass/highpass/bandpass/bandstop)
     * @param passb  passband edge frequencies [PI*rad/sample for digital filters
     *               or rad/sec. for analog filters]
     * @param stopb  stopband edge frequencies [PI*rad/sample for digital filters
     *               or rad/sec. for analog filters]
     * @param gpass  the maximum loss in the passband [dB]
     * @param gstop  the minimum attenuation in the stopband [dB]
     * @param analog true to design an analog filter, false to design a digital
     *               filter
     ...
     * @param naturalFrequencies the critical (natural) frequencies of the
     *                           filter
     * @param gpass              the maximum loss in the passband [dB]
     * @param gstop              the minimum attenuation in the stopband [dB]
     * @param analog             true to design an analog filter, false to design a digital
    ...
    protected FilterCoefficients designFilter(FilterType type, int filterOrder, double[] naturalFrequencies,
        ...
        if (!analog) {
            for (int i = 0; i < naturalFrequencies.length; i++)
                naturalFrequencies[i] = 2 * samplingFrequency
                        * Math.tan(Math.PI * naturalFrequencies[i] / samplingFrequency);
        }
    ...
    /**
     * Returns zeros, poles, and gain representation of a normalized prototype
     * analog lowpass filter which meets the given specification.
     *
     * @param filterOrder the order of the filter prototype
     * @param gpass       the maximum loss in the passband [dB]
     * @param gstop       the minimum attenuation in the stopband [dB]
     * @return zeros, poles and gain of the filter prototype which meets the
     * given specification
     */
    protected abstract FilterZerosPolesGain calculatePrototype(int filterOrder, double gpass, double gstop)
    ...
    /**
     * Calculates the natural (critical) frequencies for a given specification
     * (the 3dB frequency).
     *
     * @param type        type of the filter (lowpass/highpass/bandpass/bandstop)
     * @param filterOrder the order of the filter to be designed
     ...
     */
    protected abstract double[] calculateNaturalFrequency(FilterType type, int filterOrder, double[] passb,
                                                          double[] stopb, double gpass, double gstop, boolean analog) throws BadFilterParametersException;

    /**
     * Calculates the natural (critical) frequency for a given specification
     * (the 3dB frequency).
     *
     * @param type        type of the filter (lowpass/highpass/bandpass/bandstop)
     * @param filterOrder the order of the filter to be designed
    ...
    /**
     * Calculates the minimum filter order of the filter which would meets the
     ...
     */
    protected abstract int calculateFilterOrder(FilterType type, double[] passb, double[] stopb, double gpass,
                                                double gstop, boolean analog) throws BadFilterParametersException;
    ...
    /**
     * Finds optimal values for passband edge frequencies for an analog bandstop
     * filter (the filter with optimal values has lower order).
     *
     * @param passb passband edge frequencies [rad/sec.]
     * @param stopb stopband edge frequencies [rad/sec.]
     * @param gpass the maximum loss in the passband [dB]
    ...
    protected double[] optimizeBandstopFilterPassbandFrequencies(double[] passb, double[] stopb, double gpass,
        ...
        double passb0 = FunctionOptimizer.minimizeFunctionConstrained(objectiveFunction, passbCopy[0], stopb[0] - 1e-12,
                500);
        passbCopy[0] = passb0;

        objectiveFunction = new BandstopObjectiveFunction(1, passbCopy, stopb, gpass, gstop);
        double passb1 = FunctionOptimizer.minimizeFunctionConstrained(objectiveFunction, stopb[1] + 1e-12, passbCopy[1],
                500);
        passbCopy[1] = passb1;
    ...
    /**
     * Evaluates the bandstop objective function which is used to find optimal
     * values for passband edge frequencies (if the filter is an analog bandstop
     * filter) to minimize the order of the filter.
     *
     * @param passbandEdge              the new passband edge frequency [rad/sec.]
     * @param variablePassbandEdgeIndex specifies which passband edge to vary (0
     *                                  or 1)
     * @param passb                     passband edge frequencies [rad/sec.]
     * @param stopb                     stopband edge frequencies [rad/sec.]
     * @param gpass                     the maximum loss in the passband [dB]
     ...
     */
    protected abstract double calculateBandstopObjectiveFunctionValue(double passbandEdge,
                                                                      int variablePassbandEdgeIndex, double[] passb, double[] stopb, double gpass, double gstop);
     ...
     * bandstop filter) to minimize the filter order. Uses the values that are
     * stored in the given BandstopObjectiveFunction.
     *
     * @param passbandEdge      the new passband edge frequency [rad/sec.]
     * @param objectiveFunction the objective function which stores the values
    ...
    protected double calculateBandstopObjectiveFunctionValue(double passbandEdge,
        ...
        return calculateBandstopObjectiveFunctionValue(passbandEdge, objectiveFunction.getVariablePassbandEdgeIndex(),
                objectiveFunction.getPassb(), objectiveFunction.getStopb(), objectiveFunction.getPassbandRipple(),
                objectiveFunction.getStopbandAttenuation());
    ...
    public class BandstopObjectiveFunction implements UnivariateRealFunction {
         ...
         * Constructor.
         *
         * @param variablePassbandEdgeIndex specifies which passband edge to
         *                                  vary (0 or 1)
         * @param passb                     passband edge frequencies [rad/sec.]
         * @param stopb                     stopband edge frequencies [rad/sec.]
         * @param gpass                     the maximum loss in the passband [dB]
         * @param gstop                     the minimum attenuation in the stopband [dB]
         */
        public BandstopObjectiveFunction(int variablePassbandEdgeIndex, double[] passb, double[] stopb, double gpass,
                                         double gstop) {
        ...
        /**
         * Calculates the value of the objective function for a given new
         * passband edge frequency and values stored in this object. The value
         * of this function should be minimized to get the lowest filter order.
         *
        ...
        /**
         * Returs which passband edge this bandstop objective function will vary
         * (0 or 1)
         *
        ...
        /**
         * Returns the maximum ripple in the passband used to calculate the
        ...
        /**
         * Returns the value of the minimum attenuation in the stopband used to
         * calculate the value of this objective function
         *
    ...
    /**
     * Writes the debug information contained in the given string using the
     * program
     *
    ...
    /**
     * Enables or disables the logger that prints debug information about the
     * process of designing a filter.
     *
     * @param enable true if the logger should be enabled, false otherwise.
     */
    public void enableDebugger(boolean enable) {
        if (enable != true)
