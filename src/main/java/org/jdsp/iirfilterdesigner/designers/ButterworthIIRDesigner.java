...
public class ButterworthIIRDesigner extends AbstractIIRDesigner {
    ...
    /**
     * Returns zeros, poles, and gain of a normalized prototype analog lowpass
     * Butterworth filter which meets the given specification.
     *
    ...
    /**
     * Returns zeros, poles, and gain of a normalized prototype analog lowpass
     * filter which meets the specification.
     *
     * @param filterOrder the order of the prototype
     * @return zeros, poles and gain of the filter prototype which meets the
     * given specification
     ...
     * @param wp          passband edge frequencies [PI*rad/sample for digital filters or
     *                    rad/sec. for analog filters ]
     * @param ws          stopband edge frequencies [PI*rad/sample for digital filters or
     *                    rad/sec. for analog filters ]
     * @param gpass       the maximum loss in the passband [dB]
     * @param gstop       the minimum attenuation in the stopband [dB]
     ...
     */
    @Override
    protected double[] calculateNaturalFrequency(FilterType type, int filterOrder, double[] wp, double[] ws,
                                                 double gpass, double gstop, boolean analog) {
        ...
        if (type.isLowpass())
            ...
            WN[0] = ((passb[1] - passb[0])
                    + Math.sqrt(Math.pow((passb[1] - passb[0]), 2) + 4 * Math.pow(W0, 2) * passb[0] * passb[1]))
                    / (2 * W0);
            WN[1] = ((passb[1] - passb[0])
                    - Math.sqrt(Math.pow((passb[1] - passb[0]), 2) + 4 * Math.pow(W0, 2) * passb[0] * passb[1]))
                    / (2 * W0);
            WN[0] = Math.abs(WN[0]);
            WN[1] = Math.abs(WN[1]);
            ...
            WN[0] = W0 * (passb[1] - passb[0]) / 2.0
                    + Math.sqrt(Math.pow(-W0, 2) / 4.0 * Math.pow((passb[1] - passb[0]), 2) + passb[0] * passb[1]);
            WN[1] = -W0 * (passb[1] - passb[0]) / 2.0
                    + Math.sqrt(Math.pow(W0, 2) / 4.0 * Math.pow((passb[1] - passb[0]), 2) + passb[0] * passb[1]);
            WN[0] = Math.abs(WN[0]);
            WN[1] = Math.abs(WN[1]);
    ...
    /**
     * Calculates the minimum filter order of the filter which would meet the
     ...
     */
    @Override
    protected int calculateFilterOrder(FilterType type, double[] wp, double[] ws, double gpass, double gstop,
        ...
        int filterOrder = (int) (Math
                .ceil(Math.log10((GSTOP - 1.0) / (GPASS - 1.0)) / (2 * Math.log10(frequencyRatio))));
     ...
     * bandstop filter) to minimize the filter order.
     *
     ...
     */
    @Override
    protected double calculateBandstopObjectiveFunctionValue(double passbandEdge, int variablePassbandEdgeIndex,
                                                             double[] passb, double[] stopb, double gpass, double gstop) {

        double[] passbCopy = passb.clone();
        passbCopy[variablePassbandEdgeIndex] = passbandEdge;
        double[] possibilities = new double[2];

        possibilities[0] = stopb[0] * (passbCopy[0] - passbCopy[1])
                / (Math.pow(stopb[0], 2) - passbCopy[0] * passbCopy[1]);
        possibilities[1] = stopb[1] * (passbCopy[0] - passbCopy[1])
                / (Math.pow(stopb[1], 2) - passbCopy[0] * passbCopy[1]);
