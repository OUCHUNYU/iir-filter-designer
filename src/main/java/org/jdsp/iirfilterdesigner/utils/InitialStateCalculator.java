...
public class InitialStateCalculator {
     ...
     * Constructor
     *
     * @param filterCoefficients the coefficients of the filter for which this
     *                           calculator calculates the initial state
     */
    public InitialStateCalculator(FilterCoefficients filterCoefficients) {
        ...
        numberOfCoefficients = aCoefficients.length; // both arrays have the
        // same length now
        initialState = calculateInitialState();
    }

    /**
     * If arrays are of different lengths, this method expands the shorter array
     * with zeros so that both are of equal length.
     */
    ...
    /**
     * Grows the signal to have edges for stabilizing the filter with inverted
    ...
    public double[] growSignal(double[] signal, boolean leftOnly) {
        ...
        if (!leftOnly)
            for (int i = 0; i < edge; i++) {
                grownSignal[edge + signal.length + i] = 2 * signal[signal.length - 1] - signal[signal.length - 2 - i];
            }
